...
**WD Agent Delegation System**: Intelligent routing to specialized sub-agents using Claude Code's native Task tool.

**Agent Selection Matrix**:
```yaml
agent_routing_algorithm:
  domain_analysis: 40%     # Technical domain identification
  complexity_assessment: 25% # Task complexity and scope
  quality_requirements: 20%  # Security, performance, testing needs
  user_context: 15%         # Project history and preferences

agent_activation_thresholds:
  auto_delegate: 75%        # Automatically route to specialist
  suggest_agent: 60%        # Suggest agent but allow override
  multi_agent: 85%          # Activate multiple coordinated agents
  wave_coordination: 90%    # Trigger wave mode with agent delegation
```

**WD Specialist Agent Types**:
- **wd-frontend-agent**: UI/UX with Magic MCP, accessibility focus
- **wd-backend-agent**: APIs, databases with Context7 patterns
- **wd-security-agent**: Vulnerability assessment, compliance
- **wd-test-agent**: E2E testing with Playwright, quality assurance
- **wd-docs-agent**: Documentation, knowledge management

**Sub-Agent Delegation Decision Matrix**:

**Agent Selection Factors**:
- **Domain Keywords**: Frontend (React, Vue, CSS), Backend (API, database), Security (auth, vulnerability)
- **File Patterns**: `*.jsx|tsx` → frontend, `*auth*` → security, `*.test.*` → testing
- **Command Context**: `/wd:implement` → domain-specific, `/wd:review` → multi-agent
- **Quality Gates**: Security audits → security-agent, Performance → test-agent
...
coordination_strategy:
  agents: [wd-security-agent, wd-performance-agent, wd-quality-agent]
  execution: simultaneous
  aggregation: unified_report
  tools: [Task(frontend-specialist), Task(backend-specialist), Task(qa-specialist)]
```
...
coordination_strategy:
  workflow: implement → test → document → finalize
  agents: [wd-backend-agent, wd-test-agent, wd-docs-agent, wd-devops]
  handoff_protocol: context_preservation
  tools: [Task(backend-specialist), Task(qa-specialist), Task(general-purpose)]
```
...
trigger_conditions:
  ...
  - multi_phase_project: true
coordination_strategy:
  coordinator: Task(coordinator)
  specialists: [Task(frontend-specialist), Task(backend-specialist), Task(qa-specialist)]
  management: central_orchestration
  communication: structured_handoffs
```

**Wave Opportunity Scoring** (Enhanced with Agent Integration):
- **High Complexity >0.8**: +0.4 score, triggers coordinator agent
- **Multiple Operation Types >2**: +0.3 score, enables parallel agents
- **Critical Quality Requirements**: +0.2 score, activates qa-specialist
- **Large File Count >50**: +0.1 score, suggests parallel delegation
- **Multi-Domain Indicators**: +0.2 score, triggers specialist routing
- **Enterprise Scale**: +0.15 score, enables hierarchical coordination

**Agent-Enhanced Strategy Recommendations**:
- **Wave Score >0.7 + Agent Match >0.8**: Coordinated wave execution with specialist agents
- **Multi-Domain Tasks**: Parallel agent delegation with result synthesis
- **Sequential Workflows**: Agent pipeline with quality gate validation
- **Complex Analysis**: Hierarchical coordination with central orchestrator
- **Quality Focus**: Multi-agent validation (security + performance + testing)
...
**WD Agent-Command Routing Table**:

| Command | Complexity | Primary Agent | Secondary Agents | Coordination | Performance Gain |
|---------|------------|---------------|------------------|--------------|------------------|
| `/wd:implement` | moderate | Domain-specific | wd-test-agent | Sequential | 60% |
| `/wd:review` | high | wd-security-agent | wd-test-agent, wd-docs-agent | Parallel | 75% |
| `/wd:migrate` | high | wd-frontend/backend | wd-test-agent, wd-docs-agent | Hierarchical | 70% |
| `/wd:finalize` | moderate | wd-docs-agent | wd-security-agent | Sequential | 50% |
| `/wd:benchmark` | moderate | wd-test-agent | wd-frontend-agent | Parallel | 65% |
| `/wd:brainstorm` | complex | coordinator | Multi-domain agents | Hierarchical | 80% |

**Legacy Delegation Routing Table**:
